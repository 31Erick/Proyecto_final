---
title: "Datos policiales Costa Rica,año:2021, Erick Ruiz Mata C17013"
output:
...
```{r datos_cantones, message=FALSE, warning=FALSE}
distritos <-
  st_read(dsn = "distritos_simplificados.geojson", quiet = TRUE) %>%
  st_transform(4326)
```

```{r datos_delitos, message=FALSE, warning=FALSE}
...
**Proyecto final [Procesamineto de datos geograficos](https://gf0604-procesamientodatosgeograficos.github.io/2022-i/)**
  

 


Column {.tabset data-width=590}
-----------------------------------------------------------------------
...
```{r, Arreglo_nombre, message=FALSE, warning=FALSE}
delitos <- 
  delitos %>%
...
```{r union_delitos, message=FALSE, warning=FALSE}
delitos <-
  delitos %>%
  left_join(
    dplyr::select(
...
```{r paleta_colores, message=FALSE, warning=FALSE}
...
leaflet() %>%
  ...
  addPolygons(
    ...
    weight = 1.0,
    popup = paste(
      paste("<strong>Provincia:</strong>",distritos_delitos$provincia),
...
```{r grafico_delito, message=FALSE}
renderPlotly({
  delitos <- filtrarDelitos()
...
```{r grafico_victima, message=FALSE}
renderPlotly({
  delitos <- filtrarDelitos()
grafico_x_victima <-
  delitos %>%
...
```{r grafico_genero,message=FALSE}
renderPlotly({
  delitos <- filtrarDelitos()
